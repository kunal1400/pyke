...
function categoryNameFromString( $string ) {
    ...
    // else if( $string == "healthy" ) {
    //  $return = "gezond";
    // }
    else if( $string == "unhealthy" ) {
        $return = "ongezond";
    }
...
function generatePath( $main, $category_1, $category_2, $category_3, $sub_1, $sub_1a, $sub_1b, $sub_1c, $sub_1d, $sub_1e, $sub_1f, $sub_1g, $sub_1h, $sub_1i ) {
    ...
    // Artiyl category mapping: START   
    if( strtolower($main) == "health" ) {
        if( strtolower($category_1) == "general" ) {
            if( strtolower($sub_1a) == "general" || 
                strtolower($sub_1a) == "body" || 
                strtolower($sub_1a) == "aging" || 
                strtolower($sub_1a) == "diseases" ) {
                return 'blog>lichaam>algemeen';
            }
        }
        else if( strtolower($category_1) == "physical" ) {
            if( strtolower($sub_1b) == "general" || 
                strtolower($sub_1b) == "workout" || 
                strtolower($sub_1b) == "training" || 
                strtolower($sub_1b) == "sport" ) {
                return 'blog>physical>algemeen';
            }
        }
        else if( strtolower($category_1) == "mental" ) {
            if( strtolower($sub_1c) == "general" || 
                strtolower($sub_1c) == "strength" || 
                strtolower($sub_1c) == "psychology" || 
                strtolower($sub_1c) == "neuroscience" ) {
                return 'blog>hersenen>algemeen';
            }
        }
        else if( strtolower($category_1) == "nutrition" ) {
            if( strtolower($sub_1d) == "general" || strtolower($sub_1d) == "nutrients" ) {
                return 'blog>lichaam>algemeen';
            }
            if( strtolower($sub_1d) == "food" || strtolower($sub_1d) == "drinks" ) {
                return 'blog>mond>algemeen';
            }
        }
        else if( strtolower($category_1) == "body_care" ) {
            if( strtolower($sub_1e) == "general" ) {
                return 'blog>lichaam>algemeen';
            }
            if( strtolower($sub_1e) == "mouth" ) {
                return 'blog>mond>algemeen';
            }
            if( strtolower($sub_1e) == "skin" ) {
                return 'blog>huid>algemeen';
            }
            if( strtolower($sub_1e) == "hair" ) {
                return 'blog>haar>algemeen';
            }
        }
    }
    else if( strtolower($main) == "love" ) {        
        return 'love>'.categoryNameFromString($category_2);
    }
    else if( strtolower($main) == "wealth" ) {
        if( strtolower($category_3) == "work" ) {           
            return 'wealth>'.categoryNameFromString($category_3).'>'.categoryNameFromString($sub_1f);
        }
        else if( strtolower($category_3) == "finance" ) {
            return 'wealth>'.categoryNameFromString($category_3).'>'.categoryNameFromString($sub_1g);
        }
        else if( strtolower($category_3) == "living" ) {
            return 'wealth>'.categoryNameFromString($category_3).'>'.categoryNameFromString($sub_1h);
        }
        else if( strtolower($category_3) == "contribution" ) {
            return 'wealth>'.categoryNameFromString($category_3).'>'.categoryNameFromString($sub_1i);
        }
    }
    // END

    // HealthyIngredients category mapping: START   
    else if( strtolower($main) == "healthy" || strtolower($main) == "kids" || strtolower($main) == "shows" ) {
        $strToReturn = categoryNameFromString($main);
        if( $category_2 ) {
            $strToReturn .= '>'.categoryNameFromString($category_2);
        }
        else if( $category_1 ) {
            $strToReturn .= '>'.categoryNameFromString($category_1);
        }
        if( $sub_1 ) {
            if( $sub_1 == "healthy") {
                $strToReturn .= '>gezond';
            } else {
                $strToReturn .= '>'.categoryNameFromString($sub_1);                
            }
        }
        return $strToReturn;
    }   
    // END
        ...
        // bulletproofventures category mapping: START
        // END

        // intrestingpeopletalk category mapping: START
        // END

        // fightinsecurity category mapping: START
        // END

        // These 3 sites have only one category key i.e $category_1 so in future I just have to add keys in categoryNameFromString
...
function generateLayout( $id, $keyTakeAways, $youtubeurl ) {
    $thanksText = 'Thanks for using our key takeaways to learn something about Healthy Ingredients. For the best use of our platform we refer to the ';
    ...
    // 5,6,7,8,9,10,11,12,13
        ...
        $a = array( array(), array(), array(), array(), array(),
            ...
            array('testimonialpreset' => 'mpc_preset_20', 
                ...
                'url' => 'url:http%3A%2F%2Fmassive.mpcthemes.net%2F|title:massive.mpcthemes.net|target:%20_blank', 
                'url_font_color' => '#f3f3f3', 
                'thumbnail_size' => '100x100', 
                'thumbnail_border_css' => 'border-width:5px;border-color:#ffffff;border-style:solid;border-radius:100px;'
            ),
        ...
        $d = $a[$id];
        $return = '[vc_row enable_top_separator="true" css=".vc_custom_1563696768341{margin-top: -120px !important;}"][vc_column][vc_column_text css=".vc_custom_1563696569710{margin-bottom: 0px !important;}"]<a name="jumpvideo"></a>[/vc_column_text][/vc_column][/vc_row][vc_row css=".vc_custom_1563731120664{margin-top: 60px !important;margin-bottom: 0px !important;}"][vc_column][vc_video link="'.$youtubeurl.'"][/vc_column][/vc_row]';
        foreach ($keyTakeAways as $i => $keyTakeAway) {            
            $return .= '[vc_row css=".vc_custom_1563731109992{margin-top: 0px !important;}"][vc_column][mpc_testimonial preset="'.$d['testimonialpreset'].'" background_type="gradient" background_gradient="'.$d['background_gradient'].'" author_font_color="#ffffff" author_font_align="left" url_font_preset="mpc_preset_27" url_font_color="'.$d['url_font_color'].'" url_font_size="14" hover_url_color="#ffffff" url="'.$d['url'].'" testimonial_font_preset="mpc_preset_47" testimonial_font_color="#ffffff" testimonial_font_size="14" testimonial_font_align="left" thumbnail_size="'.$d['thumbnail_size'].'" thumbnail_border_css="'.$d['thumbnail_border_css'].'" thumbnail_padding_divider="true" thumbnail_padding_css="padding-left:0px;" border_css="border-radius:5px;" padding_css="padding:35px;"]<h2></h2><h3>KEY TAKEAWAYS</h3><h4>'.$keyTakeAway['key'].'</h4>'.$keyTakeAway['text'].'<h4 style="text-align: center;">[linktimestamp link=#jumpvideo time='.$keyTakeAway['timestamp'].' color=#ffffff icon=fa-youtube-play]Play the rest of part[/linktimestamp]</h4>[/mpc_testimonial][/vc_column][/vc_row]';
        }
        $return .= '[vc_row][vc_column width="1/4" css=".'.$d['css'].'{margin-bottom: 20px !important;}"][mpc_image image="'.$mpcNextImageId.'" padding_divider="true" margin_divider="true" image_size="100x100" image_opacity="100" image_border_css="border-radius:75px;" image_inner_border_gap="0" effect="none" image_hover_opacity="100"][/vc_column][vc_column width="3/4"][mpc_textblock content_width="100" font_preset="preset_2" '.$thanksFontSize.']'.$thanksText.' '.$linkWithSpan.' . '.$rememberTxt.'[/mpc_textblock][/vc_column][/vc_row]';
    }
    ...
    if( $id == 25 ) {
        ...
        $return = '[vc_row enable_top_separator="true" css=".vc_custom_1563696768341{margin-top: -120px !important;}"][vc_column][vc_column_text css=".vc_custom_1563696569710{margin-bottom: 0px !important;}"]<a name="jumpvideo"></a>[/vc_column_text][/vc_column][/vc_row][vc_row css=".vc_custom_1564676601456{margin-top: 60px !important;margin-bottom: 0px !important;}"][vc_column][vc_video link="'.$youtubeurl.'"][/vc_column][/vc_row][vc_row][vc_column][vc_row_inner][vc_column_inner width="1/4"][mpc_image image="'.$mpcImageId.'" image_opacity="100" image_inner_border_gap="0" effect="none" image_hover_opacity="100" mpc_ribbon__preset="mpc_preset_16" mpc_ribbon__style="fullwidth" mpc_ribbon__corners_color="#4a7c8e" mpc_ribbon__font_preset="mpc_preset_17" mpc_ribbon__font_color="#f7f7f7" mpc_ribbon__font_size="22" mpc_ribbon__font_line_height="1.7" mpc_ribbon__text="key takeaways" mpc_ribbon__background_type="gradient" mpc_ribbon__background_gradient="#402312||#cce0f2||50;50||225||linear" mpc_ribbon__padding_divider="true" mpc_ribbon__padding_css="padding-top:15px;padding-bottom:15px;" mpc_ribbon__margin_divider="true" mpc_ribbon__margin_css="margin-top:180px;"][/vc_column_inner][vc_column_inner width="3/4"][mpc_textblock preset="preset_0" content_width="100" font_preset="preset_2" font_color="#cce0f2" font_size="35" font_line_height="1" font_align="left" mpc_tooltip__disable="true" mpc_tooltip__preset="mpc_preset_29"]<h1><span style="color: #402312;">key</span> takeaways</h1>[/mpc_textblock][mpc_divider lines_color="#cce0f2" lines_weight="5" margin_divider="true" margin_css="margin-top:20px;margin-bottom:20px;"][/vc_column_inner][/vc_row_inner][/vc_column][/vc_row][vc_row][vc_column]';
        $keyTakeAwaysCheckbox = '';
        foreach ($keyTakeAways as $i => $keyTakeAway) {
    ...
    if( $id == 29 ) {
        ...
        $return = '[vc_row enable_top_separator="true" css=".vc_custom_1563696768341{margin-top: -120px !important;}"][vc_column][vc_column_text css=".vc_custom_1563696569710{margin-bottom: 0px !important;}"]<a name="jumpvideo"></a>[/vc_column_text][/vc_column][/vc_row][vc_row css=".vc_custom_1564676663096{margin-top: 60px !important;margin-bottom: 0px !important;}"][vc_column][vc_video link="'.$youtubeurl.'"][/vc_column][/vc_row][vc_row][vc_column][vc_row_inner][vc_column_inner width="1/4"][mpc_image image="'.$mpcImageId.'" image_opacity="100" image_inner_border_gap="0" effect="none" image_hover_opacity="100" mpc_ribbon__preset="mpc_preset_16" mpc_ribbon__style="fullwidth" mpc_ribbon__corners_color="#4a7c8e" mpc_ribbon__font_preset="mpc_preset_17" mpc_ribbon__font_color="#f7f7f7" mpc_ribbon__font_size="22" mpc_ribbon__font_line_height="1.7" mpc_ribbon__text="key takeaways" mpc_ribbon__background_type="gradient" mpc_ribbon__background_gradient="#402312||#cdcbce||50;50||225||linear" mpc_ribbon__padding_divider="true" mpc_ribbon__padding_css="padding-top:15px;padding-bottom:15px;" mpc_ribbon__margin_divider="true" mpc_ribbon__margin_css="margin-top:180px;"][/vc_column_inner][vc_column_inner width="3/4"][mpc_textblock preset="preset_0" content_width="100" font_preset="preset_2" font_color="#cdcbce" font_size="35" font_line_height="1" font_align="left" mpc_tooltip__disable="true" mpc_tooltip__preset="mpc_preset_29"]<h1><span style="color: #402312;">key</span> takeaways</h1>[/mpc_textblock][mpc_divider lines_color="#cdcbce" lines_weight="5" margin_divider="true" margin_css="margin-top:20px;margin-bottom:20px;"][/vc_column_inner][/vc_row_inner][/vc_column][/vc_row][vc_row][vc_column]';
        $keyTakeAwaysCheckbox = '';
        foreach ($keyTakeAways as $i => $keyTakeAway) {
    ...
    if( $id == 33 ) {
        ...
        $return = '[vc_row enable_top_separator="true" css=".vc_custom_1563696768341{margin-top: -120px !important;}"][vc_column][vc_column_text css=".vc_custom_1563696569710{margin-bottom: 0px !important;}"]<a name="jumpvideo"></a>[/vc_column_text][/vc_column][/vc_row][vc_row css=".vc_custom_1564676632015{margin-top: 60px !important;margin-bottom: 0px !important;}"][vc_column][vc_video link="'.$youtubeurl.'"][/vc_column][/vc_row][vc_row][vc_column][vc_row_inner][vc_column_inner width="1/4"][mpc_image image="'.$mpcImageId.'" image_opacity="100" image_inner_border_gap="0" effect="none" image_hover_opacity="100" mpc_ribbon__preset="mpc_preset_16" mpc_ribbon__style="fullwidth" mpc_ribbon__corners_color="#4a7c8e" mpc_ribbon__font_preset="mpc_preset_17" mpc_ribbon__font_color="#f7f7f7" mpc_ribbon__font_size="22" mpc_ribbon__font_line_height="1.7" mpc_ribbon__text="key takeaways" mpc_ribbon__background_type="gradient" mpc_ribbon__background_gradient="#402312||#cea791||50;50||225||linear" mpc_ribbon__padding_divider="true" mpc_ribbon__padding_css="padding-top:15px;padding-bottom:15px;" mpc_ribbon__margin_divider="true" mpc_ribbon__margin_css="margin-top:180px;"][/vc_column_inner][vc_column_inner width="3/4"][mpc_textblock preset="preset_0" content_width="100" font_preset="preset_2" font_color="#cea791" font_size="35" font_line_height="1" font_align="left" mpc_tooltip__disable="true" mpc_tooltip__preset="mpc_preset_29"]<h1><span style="color: #402312;">key</span> takeaways</h1>[/mpc_textblock][mpc_divider lines_color="#cea791" lines_weight="5" margin_divider="true" margin_css="margin-top:20px;margin-bottom:20px;"][/vc_column_inner][/vc_row_inner][/vc_column][/vc_row][vc_row][vc_column]';
        $keyTakeAwaysCheckbox = '';
        foreach ($keyTakeAways as $i => $keyTakeAway) {
